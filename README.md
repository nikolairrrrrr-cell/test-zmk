...
|---|---|---|
| `pdf-zmk` | Извлечение данных из чертежей/изображений типа PDF_ZMK | `База2` (переименование: `GOOGLE_SHEET_PDF_ZMK`) |
| `pdf-zmk2` | Тип чертежа PDF_ZMK2; JSON с кропа; вход — папка **`СЕ1`** | Лист Google **`СЕ1`**. Инструкция: [docs/agent-instrukciya-se1.md](docs/agent-instrukciya-se1.md) |
| `ves` | Парсинг веса с сайтов, кэш, выгрузка | `Масса` (`GOOGLE_SHEET_PDF_ZMK_VES`) |
...
Один JSON или один исходник → **полная перезапись** листа **`СЕ1`** только этими строками.
...
Папка с файлами → **как `pdf-zmk full`**: все валидные payload’ы склеиваются (разделитель — пустая строка) и **один раз** пишутся на лист **`СЕ1`**.

**По умолчанию** входная папка — **`СЕ1`** в корне проекта (можно переопределить: `--input-dir` или env **`PDF_ZMK2_INPUT_DIR`**). В **`СЕ1`** держите исходные изображения/PDF; **`data/recognition/<slug>.json`** и **`.table_x4.jpg`** — как у База2, не в **`СЕ1`**.

```bash
# macOS/Linux — без аргументов: обработать всё из папки СЕ1
python3 -m app.cli pdf-zmk2 full --sheet-write

# Windows
py -m app.cli pdf-zmk2 full --sheet-write

# Явно другая папка
py -m app.cli pdf-zmk2 full --input-dir "ДругаяПапка" --sheet-write
```

Флаги: `--input-dir` (необязательно), `--sheet-write`, `--dry-run`, `--workers`, `--output-json`.

Скрипты `scripts/run-tools.sh` / `run-tools.ps1`: **без второго аргумента** — `pdf-zmk2 full` с папкой **`СЕ1`**; если передать **каталог** — `pdf-zmk2 full --input-dir …`; если **файл** — `pdf-zmk2 run`.
...
Phase 4: NORMALIZE + SHEETS
  ...
  → загрузка всех JSON → нормализация → запись в Google Sheet «База2» (или `GOOGLE_SHEET_PDF_ZMK`)
```
...
./scripts/run-tools.sh pdf-zmk2          # всё из папки СЕ1
./scripts/run-tools.sh ves

# Windows PowerShell
.\scripts\run-tools.ps1 -Command pdf-zmk -InputPath new
.\scripts\run-tools.ps1 -Command pdf-zmk2   # всё из папки СЕ1
