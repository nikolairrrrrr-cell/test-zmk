...
## Purpose
Resolve support-position weights using scraper sources, cache results in local DB, optionally write table to Google Sheets `PDF_ZMK_VES`.
...
- Optional write to Google Sheet `PDF_ZMK_VES`.
...
- If `--sheet-write` and not `--dry-run`: clears and updates `PDF_ZMK_VES!A:Z`.
